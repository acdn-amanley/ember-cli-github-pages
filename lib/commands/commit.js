  ...
  run: function(options, rawArgs) {
    ...
    function addAndCommit() {
      return runCommand('git -c core.safecrlf=false add "' + options.destination + '"', execOptions)
              .then(function() {
                return runCommand(`if [[ -n $(git status -s) ]]; then git commit -m "${options.message}" ${options.noverify ? '-n' : ''}; else echo 'No changes to git-page, skipping commit.'; fi`, execOptions);
              })
    }
