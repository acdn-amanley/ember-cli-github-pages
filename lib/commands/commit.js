...
module.exports = {
  ...
  },
  {
    ...
    type: Boolean,
    default: false,
    description: 'Skip verifying the commit'
  }],
  ...
  run: function (options, rawArgs) {
    ...
    var execOptions = { cwd: root, shell: true };

    function buildApp() {
      var env = options.environment;
      return runSpawnCommand('ember build --environment=' + env, [], execOptions);
    }
    ...
    function addAndCommit() {
      return runCommand('git -c core.safecrlf=false add "' + options.destination + '"', execOptions)
        .then(function () {
          return runCommand('git commit -m "' + options.message + '" --no-verify', execOptions)
        })
    }
...
function runSpawnCommand(/* child_process.exec args */) {
  var args = Array.prototype.slice.call(arguments);
  var lastIndex = args.length - 1;
  var lastArg = args[lastIndex];
  var logOutput = false;
  ...
  return new RSVP.Promise(function (resolve, reject) {
    const proc = spawn.apply(spawn, args)
